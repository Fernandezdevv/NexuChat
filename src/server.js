    ...
    if (plano === 'mensal') {
        try {
            const paymentData = {
                ...
                description: 'Assinatura Mensal NexusChat',
            ...
            // Retorna os dados do QR Code para o seu HTML
            return res.json({
                id: result.id,
            ...
            console.error("Erro PIX Direto:", error);
            return res.status(500).json({ error: "Falha ao gerar PIX" });
        }
    }

    // LÓGICA ANUAL: Leva para o Mercado Pago (para parcelar no cartão)
    try {
        const preferenceData = {
            items: [{ title: 'Assinatura Anual NexusChat', quantity: 1, unit_price: valor, currency_id: 'BRL' }],
            payer: { email: emailLimpo },
            external_reference: emailLimpo,
            notification_url: `${urlBase}/webhook`,
            back_urls: { success: `${urlBase}/registrar?email=${emailLimpo}` },
...
app.get('/reembolso', (req, res) => res.sendFile(path.join(__dirname, '../public/reembolso.html')));
app.listen(port, () => {
